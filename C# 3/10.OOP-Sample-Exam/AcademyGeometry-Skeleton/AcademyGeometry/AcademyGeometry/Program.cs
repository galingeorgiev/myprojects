...
namespace GeometryAPI
{
    ...
    public class Circle : Figure, IAreaMeasurable, IFlat
    {
        ...
        public Circle(Vector3D vector, double radius) : base(vector)
        {
            this.Radius = radius;
        }

        public Vector3D CenterVector
        {
            get { return this.vertices[0]; }
            set { this.vertices[0] = value; }
        }
    ...
    class Cylinder : Figure, IFlat, IAreaMeasurable, IVolumeMeasurable
    {
        public double Radius { get; private set; }

        public Cylinder(Vector3D a, Vector3D b, double radius) : base(a,b)
        {
            this.Radius = radius;
        }
        ...
        public virtual void ExecuteFigureCreationCommand(string[] splitFigString)
        {
            switch (splitFigString[0])
            {
                ...
                case "circle":
                    {
                        Vector3D a = Vector3D.Parse(splitFigString[1]);
                        double radius = double.Parse(splitFigString[splitFigString.Length - 1]);
                ...
                case "area":
                    {
                        ...
                        {
                            Console.WriteLine("{0:f2}", (this.currentFigure as IAreaMeasurable).GetArea());
                        }
                        else
                        {
                ...
                case "volume":
                    {
                        ...
                        {
                            Console.WriteLine("{0:f2}", (this.currentFigure as IVolumeMeasurable).GetVolume());
                        }
                        else
                        {
                            Console.WriteLine("undefined");
                ...
                case "normal":
                    {
                        ...
                        {
                            Console.WriteLine("{0:f2}", (this.currentFigure as IFlat).GetNormal());
                        }
                        else
                        {
                            Console.WriteLine("undefined");
